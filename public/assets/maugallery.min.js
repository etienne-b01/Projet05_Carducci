...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){var fullSizeSrc=element.data("full-size-src");$(`#${lightboxId}`).find(".lightboxImage").attr("src",fullSizeSrc);$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImageSrc=$(".lightboxImage").attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];$(".gallery-item").each(function(){let fullSizeSrc=$(this).data("full-size-src");if(activeTag==="all"||$(this).data("gallery-tag")===activeTag){imagesCollection.push(fullSizeSrc)}});let index=imagesCollection.indexOf(activeImageSrc);if(index===-1){index=imagesCollection.length-1}else if(index===0){index=imagesCollection.length-1}else{index--}
$(".lightboxImage").attr("src",imagesCollection[index])},nextImage(){let activeImageSrc=$(".lightboxImage").attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];$(".gallery-item").each(function(){let fullSizeSrc=$(this).data("full-size-src");if(activeTag==="all"||$(this).data("gallery-tag")===activeTag){imagesCollection.push(fullSizeSrc)}});let index=imagesCollection.indexOf(activeImageSrc);if(index===-1||index===imagesCollection.length-1){index=0}else{index++}
$(".lightboxImage").attr("src",imagesCollection[index])},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
...
$(".active-tag").removeClass("active active-tag");$(this).addClass("active-tag active");var tag=$(this).data("images-toggle");$(".gallery-item").each(function(){$(this).parents(".item-column").hide();if(tag==="all"){$(this).parents(".item-column").show(300)}else if($(this).data("gallery-tag")===tag){$(this).parents(".item-column").show(300)}})},}})(jQuery)
